...
<h3 id=hint>Hints for MF1–MF2</h3>
...
<div class="spoiler">
    ...
    <ul>
        <li>Partition the image in smaller, partially overlapping <strong>blocks</strong>. For example, if your window size is 21 × 21, a reasonable block size might be 50 × 50. Solve the median filter problem separately for each block; place the blocks so that each output pixel comes from exactly one block. Be careful with the boundaries.</li>
    ...
    if ny is not None and nx is not None and hy is not None and hx is not None:
        ...
        if data is not None:
            ...
            for y in range(ny):
                expl += ' '
                for x in range(nx):
                    expl += f' {safeprint(safeget(data, y, x)):>11s}'
                expl += '\n'
            expl += '\n'
